...
const BodegasDisponibles = () => {
  ...
  const obtenerBodegasDisponibles = async () => {
    try {
      ...
      // Filtrar solo las bodegas con status DISPONIBLE
      const disponibles = response.data.filter(bodega => bodega.status === "DISPONIBLE");
      ...
      Swal.fire("Error", "No se pudieron cargar las bodegas disponibles", "error");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="pt-10 px-4 pb-5 w-full">
        <h2 className="text-black text-2xl font-semibold mb-6">Bodegas disponibles</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {bodegas.length === 0 ? (
            ...
            bodegas.map((bodega) => (
              <div key={bodega.id} className="bg-white rounded-lg p-6 shadow-lg">
                <h3 className="text-black text-lg font-bold mb-2">Bodega #{bodega.folio}</h3>
